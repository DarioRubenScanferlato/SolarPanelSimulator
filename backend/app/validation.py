...
from app.constants import (
    ...
    PANEL_COUNT_MIN,
    PANEL_AREA_MIN,
    PANEL_EFFICIENCY_MIN, PANEL_EFFICIENCY_MAX,
...
def validate_input(input_data: SolarInput) -> list[ValidationError]:
    ...
    if input_data.panel_count < PANEL_COUNT_MIN:
        errors.append(ValidationError(
            field="panel_count",
            message=f"Panel count must be at least {PANEL_COUNT_MIN}"
        ))
    ...
    if input_data.panel_area_m2 < PANEL_AREA_MIN:
        errors.append(ValidationError(
            field="panel_area_m2",
            message=f"Panel area must be at least {PANEL_AREA_MIN} m²"
        ))
    ...
    if input_data.duration_days < DURATION_DAYS_MIN:
        errors.append(ValidationError(
            field="duration_days",
            message=f"Duration must be at least {DURATION_DAYS_MIN} day"
        ))
